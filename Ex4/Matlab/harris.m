...
%Ix=filter2([-2 -1 0 1 2],img,'same');  %filter2(gx,img,'same');
%Iy=filter2([-2 -1 0 1 2]',img,'same');  %filter2(gy,img,'same');
Ix=filter2(gx,img,'same');  %filter2(gx,img,'same');
Iy=filter2(gy,img,'same');  %filter2(gy,img,'same');
...
clear Ix Iy Ix2Sm Iy2Sm IxIySm C1 C2 C4 detC traceC
