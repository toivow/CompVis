...
while N > sample_count:
    ...
    p2 = points_h[:,id2]
    l = np.cross(p1, p2)
    l = l/np.sqrt(np.sum( [l[0]**2, l[1]**2]))
    ##-your-code-ends-here-##
    ...
    ##-your-code-starts-here-##
    inliers = []
    for point_id in np.arange(len(x)):
        if abs(np.dot(l, points_h[:,point_id])) < t:
            inliers.append(point_id)
    ##-your-code-ends-here-##
...
y_inliers = []
for point_id in np.arange(len(x)):
    if abs(np.dot(best_line, points_h[:,point_id])) < t:
        x_inliers.append(x[point_id])
        y_inliers.append(y[point_id])
##-your-code-ends-here-##  
