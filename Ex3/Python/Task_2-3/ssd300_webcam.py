...
while True:
    ...
    ratiox = img.shape[1]/img_width
    ratioy = img.shape[0]/img_height

    image_res = imresize(img, (img_height, img_width, 3), preserve_range=True)
    image_res = np.array([image_res])
    with tf.device("/gpu:0"):
        predictions = model.predict(image_res)

    # Removes all zero confidence predictions
    predictions = predictions[0][np.transpose(predictions[0][:,1] != 0), :]


    for (class_id, confidence, xmin, ymin, xmax, ymax) in predictions:
        if confidence > 0:
            xmin = round(xmin*ratiox)
            ymin = round(ymin*ratioy)
            xmax = round(xmax*ratiox)
            ymax = round(ymax*ratioy)
            img = cv2.rectangle(img, (xmin, ymin), (xmax, ymax), (0, 255, 0), 2)
            cv2.putText(img, classes[int(class_id)], (xmin, ymin), cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.8, color=(0, 255, 0))
        else:
            continue
