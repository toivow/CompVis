...
% Size of the third dimension is 3, because there are 3 channels for each
...
% Yes, the difference each run is due to the filter bank being random.
% size(y) = 380 508 10
% size(x) = 384 512 3
% size(w) = 5 5 3 10
%
% The filter is of size 5x5x3. The input image is a three channel image,
% the size being 348x512. The output size depends on a few variables in the
% convolution operator and because it cannot filter from the corner,
% and thus it needs to start from 2 coordinates below in rowwise and
% columnwise.
%
...
% size(y_pad) = 384 512 10
% This time the image is the same size because padding adds the necessary
% zeros so that the convolution filtering doesn't reduce the size.
%
...
% The repmat function repeats this laplacian filter for every color
% channel (ie. Red Green Blue). With this the edge detection can be done
% for every color. The filter detects the edges/outlines of the objects in
% the image. It detects color rapid color changes in the image.
...
% The non-linear functions allow the CNN to get actually interesting
% values, as without non-linear functions the network would be just a bunch
% of vector products and vector sums, which could be represented with just
% one layer. Because of this we cannot create as complex of an
% identification with just linear operators.
%
%% Pooling
...
% 
% Max pooling downsamples the image to get more general features from the
% image. Thus we can use the convolution on the subregions as well.
% 
% Stride value determines how many densely the filter goes over the image.
% When stride = 1 the filter jumps from coordinate (0, 0) to (0, 1), but
% when stride = 2 it goes from (0, 0) to (0, 2). Stride can be used to
% counteract paddigns effect of largening the image.
% 
...
%   
% In an ideal case the histograms would be completely classified, thus the
% positive values being completely over the latter blue line and the
% negative values completely below the earlier line. This means that all
% values are classified correctly, and that none would be between them,
% thus being unclassified.
% 
% 
...
%    Which one? 
% 
% Laplacian filter, as the values in the center are very negative, and the
% outskirts are more positive, thus finding edges.
%
%
%
...
%
% Increasing the learning rate has the positive effect of finding the right
% direction faster, but the issue is that it doesn't find the optimal
% solution and instead oscillates between different values. This can be
% seen as the error value oscillating in the graph.
%
% Momentum allows the network to jump over local minimas. With the value 0, the error 
% value oscillates again, although the general trend goes downwards. In
% practice it speeds up learning.
% 
